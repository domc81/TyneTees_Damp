...
- `enquiries` has `won_at` (set when deposit marked paid) and `cf_exported_at` (set on CF CSV download) columns. Status flow: accepted → completed → `handed_over` (terminal — job exported to Contractor Foreman). Handover pack page at `/survey/[projectId]/handover/` provides all CF exports.
