...
- **`surveys.cf_project_reference` is manual free text** — CF project IDs are created by hand in Contractor Foreman; never auto-generate or validate a format. `CfReferencePrompt` (fires at the office win touchpoints: Kanban→Won, drawer status→Won, deposit marked paid) is a soft Save/Skip nudge — never make it a hard gate; the field stays editable on the survey hub's Survey Details card.
...
- **The Operations material list comes from `lib/material-purchase-list.ts` and is parity-gated** against the damp workbook's `Material-List` sheet (cellmap `material_list` block, rows 13-59): each SKU rule transcribes a Qty cell's pack-rounding formula (CEILING to 5m membrane cuts / 50-plug / 20-plug multiples, ROUNDUP to rolls/bags/tubs, DPC cream rounds to NEAREST cartridge with a min of 1) over engine line quantities keyed by template `line_key` (now threaded onto `CalculatedLine`). Damp jobs include `site_preparation` lines (`dampPurchaseSourceLines()` — Antinox lives there). Never revert the list to raw `inputQuantity` displays, and never invent purchase rules for timber/woodworm/condensation — their workbook "Sub Contractor Mats" sheets are literally "TBC"; those types show measurement quantities with UOM. Operations/measurement displays split warmline lines into a `warmline_iwi` pseudo-section (the workbook's own Sub Contractor Costs tab does the same subtraction).
