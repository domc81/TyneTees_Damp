...
5. **Admin edits are hardened (2026-07-11, migration 7):** writes to the three admin areas' tables are admin-role-only at the DB (RLS `is_pricing_admin()`), every change lands in `pricing_change_log` with actor + old/new values (Change History panel on each page), and every save runs the reference-job smoke check against `pricing_smoke_baselines`. An intended change shows as a small attributed delta to accept; a fat-finger shows as a large one to reject before customers are quoted it.
6. **The smoke check is not parity.** It diffs the platform against its own last-accepted prices (correct yardstick post-go-live). Structural changes still go through the parity harness first, then a baseline refresh (`scripts/smoke/seed-baselines.ts`).
