...
- **T15** *(tablet layout)*: the wizard has only ever been used at phone widths in the field. Walk all 5 wizard steps, PhotoCapture (camera + gallery + description/visibility modal), AudioRecorder, and the sync pill at tablet widths — DevTools responsive mode at 768×1024 and 1024×768 (portrait and landscape), plus a real Android tablet if TTDP buys one. Check for: over-stretched full-width inputs, the stepper and nav buttons at intermediate breakpoints, photo grid density, and modal sizing. Cosmetic issues → fix in Phase 7; anything blocking input → fix before the field pilot.
