...
The happy path is genuinely good: the drawer's NEXT panel tells staff what to do at every stage, statuses auto-advance, notifications fire, and the wizard's room-first flow is well-structured on a phone. But the audit found **two flow-breaking P0s**: report generation never starts (the report page spins forever, which also blocks Approve & Send — the office cannot send anything to a customer), and the wizard's Polish button calls an API route that doesn't exist. Both fail **silently**, which is the recurring theme: Approve & Send's helpful server error is shown as an empty toast, and stage jumps in the Kanban happen with no confirmation or feedback. A third apparent gap — the customer is never emailed the £150 payment link at booking — turned out to be **by design** (external payment system; office marks paid manually; see P1-1), leaving a small labelling suggestion rather than a hazard.

**Counts (final):** 2× P0 ✅ fixed · P1: 3 fixed (P1-3/4/5), 1 by-design (P1-1), 1 fixed-during-run (P1-2), 1 withdrawn (P1-6) · 8× P2 ✅ fixed · 3× P3 ✅ fixed. **Audit closed.**

**Update 2026-07-05 (later same day):** all four actionable P1s resolved and verified live — P1-3 send preflight, P1-4 move guards + `won_at` integrity, P1-5 photo-tier management. P1-6 withdrawn (already-correct query; the culprit is duplicate "Steven Robinson" profiles — data hygiene, not code).

**Update 2026-07-05 (evening):** all 8 P2s and 3 P3s fixed in commit `4ff6fb8` and spot-verified live (sidebar identity card, Lead Pipeline title/counter, all-at-once New Lead validation incl. email/postcode format checks). Decisions taken with Dominic: office CAN edit surveyor working hours (P2-1); post-Complete-Survey landing is role-based — surveyor confirmation screen with View Costing link, office/admin to costing (P2-8); terminology is "Lead" for the pipeline object and "Booked" for the `scheduled` booking state (P3-3). P2-2 fixed at root: SlotPicker now backfills the surveyor name for `defaultSurveyorId` (also fixes the calendar reschedule modal). P3-1 fixed at render time via `lib/status-labels.ts` so historic activity rows display correctly too.
**Single most important fix:** make report generation actually fire (P0-1) — everything customer-facing is downstream of it.

**Fixes applied during/after the audit run** (commits alongside this report): both P0 root causes (deprecated OpenRouter model → `google/gemini-2.5-flash`; report-generation fetch pointed at `localhost:3000` → relative URL) and the P1-2 email constraint (migration `20260705000002`). Post-deploy verification: the report page now generates and renders a full report in ~3 s. **One action needs Dominic:** the TTDP OpenRouter account's privacy settings currently block all models, so LLM narrative/polish stay on fallback text until https://openrouter.ai/settings/privacy is updated (or the app switches to the shared DC81 key). All other findings are recommendations only.
...
| P1-1 | ~~P1~~ by design | Booking / payments | Customer is never emailed the survey-fee payment link at booking — **confirmed intentional** (client takes payment through an external system; office marks paid manually; deposits are taken in Contractor Foreman after handover). Residual P3: the unused `/api/payments/send-link` route + `/pay/[token]` page are dormant code |
...
| P1-3 | ✅ fixed | Enquiry drawer | Approve & Send failure leaves no durable feedback — now preflights the report and shows an inline amber explainer + Re-check; server errors persist inline. Verified live 2026-07-05. |
| P1-4 | ✅ fixed | Kanban board | Unguarded stage jumps + Won left `won_at` NULL — every manual move (drag, card dropdown, drawer pill) now confirms with a consequence note and success toast; `won_at` is stamped on entering Won and cleared on leaving. Verified live 2026-07-05. |
| P1-5 | ✅ fixed | Wizard photos | Photo tiers invisible/immutable after upload — thumbnails now show an always-visible tier badge; tap opens an edit modal (change description/visibility or delete); delete button visible on touch. Verified live 2026-07-05. |
| P1-6 | withdrawn | Booking wizard | Not a code bug — `SurveyorSelect` already filters `is_active AND is_surveyor`. The audit matched the display name of one of three duplicate "Steven Robinson" profiles (a data-hygiene issue, noted below). |
...
- **Fix applied during audit:** both fetches switched to relative `/api/generate-report`; model switched to `google/gemini-2.5-flash`. **Post-deploy verification:** report page now completes in ~3 s and produces a full, editable report (`verify-01-report-page.png`) — the P0 dead end is gone. Remaining: the executive summary still falls back to placeholder text because the **TTDP OpenRouter account's privacy/data-policy settings block every model tested** (gemini, claude, llama, qwen, all grok) with "No endpoints available matching your guardrail restrictions and data policy". ➜ **Action for Dominic:** open https://openrouter.ai/settings/privacy on the TTDP OpenRouter account (key in `.env.local`, £10/week limit) and enable the provider policy, or switch the app to the shared DC81 key. Residual recommendations: surface generation failures on the page (add an AbortController timeout like the polish route has), and set `NEXT_PUBLIC_APP_URL` in the Coolify env (also used by handover-pack and payment-link emails, which currently fall back to `''`).
...
- **Fix applied during audit:** model switched to `google/gemini-2.5-flash` (route.ts:139). **Still blocked** by the TTDP OpenRouter account's data-policy settings (see P0-1 action) — until those are changed, Polish keeps failing. Residual recommendation: the wizard's Polish click handlers (`components/wizard/ExternalInspectionStep.tsx:71`, `RoomInspectionStep.tsx:115`) must show an error toast on non-OK responses — the silent no-op is what turned an API failure into a dead-feeling button.

### P1-1 — Customer never receives the survey-fee payment link *(RESOLVED: by design — not a defect)*
- **Where:** Convert & Book completion → enquiry in "Awaiting Payment" (journey 5)
- **Original observation:** booking created the £150 `survey_fee` payment row (token generated) but `communication_log` has zero rows — no email sent at any point; the drawer offers only "Mark as Paid" with no send/copy affordance.
- **Post-audit clarification from Dominic (2026-07-05):** this is **intentional**. The client takes payment through an external payment system; office manually marks survey fees paid here. Deposits for the actual works are never taken on this platform — they are collected in Contractor Foreman after handover. The `/pay/[token]` page and `/api/payments/send-link` route are built-ahead infrastructure, deliberately not wired up.
- **Residual (P3):** if the send-link path is ever activated, note its `appUrl` falls back to `''` when `NEXT_PUBLIC_APP_URL`/`NEXT_PUBLIC_SITE_URL` are unset (route.ts:110) — the emailed link would be a broken relative path. Also consider whether the drawer should say "payment collected externally — mark paid when received" so new staff don't wait for an email that never sends.
- **Evidence:** `j5-24-after-continue.png`, `j5-26-records.png`; DB captures in transcript.
